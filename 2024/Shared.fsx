...
module Parser =
    ...
    let rec manyTill parser input =
        let firstResult = run parser input

        match firstResult with
        | Success(firstValue, remainingInput) -> Some(firstValue, remainingInput)
        | Failure err ->
            if err = "No more input" then
                None
            else
                manyTill parser input[1..]

    let skipManyTill1 parser =
        let innerFn input =
            let result = manyTill parser input

            match result with
            | None -> Failure("Not found")
            | Some(s, r) -> Success(s, r)
